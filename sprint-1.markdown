...
| 08-21 | 금 | **인프라**<br>· AWS 루트 계정으로 로그인 후 IAM 계정 생성, IAM 계정으로 재로그인<br>· RDS 접속 URL(엔드포인트) 생성<br><br>**문서 · 사이트**<br>· 역할 분담 확정 — 테일러 PM / 왓슨 Flutter / 앨리스 파이프라인 / 토마스 에이전트<br>· 4장 1)을 애자일 스크럼으로 재작성 — 스프린트 5회 · 칸반 보드 · 스크럼 운영 규칙<br>· 목차 하위 18개 항목에 앵커 링크 연결<br>· 사업명 정본을 표지 제목으로 확정하고 `_config.yml` · 1장 · `CLAUDE.md` 일치<br>· RFP 하네스 작성 (`.claude/rules/rfp-harness.md`) — 요구사항 11개 분류 체계 · 추적성 규칙 · PMR<br>· **클라이언트 스택 Flutter 확정**, R&R은 4장 2) 배정 유지 — 참고 자료의 Next.js/Vercel · FE-BE 재배정 안은 미채택<br>· **벡터 저장소 Aurora pgvector 확정** — 참고 자료의 OpenSearch/Pinecone 미채택, 별도 벡터 DB 미도입<br>· **Flutter 빌드 대상 Android · iOS 확정** — 웹 타깃 제외<br>· **기능 우선순위 필수/선택 2단계 확정** — 2장 5)에 등급 정의 기재, 3단계 이상 미도입<br>· 커밋 4건 작성 후 `origin/main` 푸시. `.env` 계열 `.gitignore` 등록<br>· **2장 개발 요구 사항 본문 작성** — 1)·2)·5)·6) 완료, 요구 ID(`SFR`/`PER`/`SER`) 부여. 3) 4) 7)은 업무·발주처 확인 대기<br>· **1장 2) 주요 사업 내용 작성** — 사업 범위 · 세부 과제 4건(2장 대응 부여) · 대상 시스템 및 데이터. 5번 과제와 연계 기관·기존 시스템은 미확정으로 비워 둠<br>· **1장 3) 기대 효과 작성** — 정성적 효과 4건, 정량 성능 목표 3건. 업무 효과 정량 지표는 산출 근거 없어 미기재<br>· `fin.ragtailor.com` DNS 사전 생성 조사 — Cloudflare 확인, 소유 확인 TXT 선행 필요 판단<br>· 이 페이지 작성 | 4장 1) · 2) 본문<br>1장 2) · 3) 본문<br>2장 1)·2)·5)·6) 본문<br>칸반 보드<br>RFP 하네스<br>개발 로그 1건<br>커밋 8건 푸시 | 완료 |
| 08-22 | 토 | *작성 예정* | | 대기 |
| 08-23 | 일 | *작성 예정* | | 대기 |
| 08-24 | 월 | *작성 예정* | | 대기 |
| 08-25 | 화 | *작성 예정* | | 대기 |
